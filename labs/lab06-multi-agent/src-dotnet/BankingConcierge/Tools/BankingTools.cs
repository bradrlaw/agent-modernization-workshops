...
using BankingConcierge.Data;

namespace BankingConcierge.Tools;

// Framework-agnostic banking tools (identical domain logic to Lab 03). Each method
// returns a JSON string. In Lab 06 these are wrapped as AIFunctions in AgentTeam.cs
// and handed to the specialist agents. [Description] attributes feed the tool schema
// the model sees, so it knows when and how to call each one.
public static class BankingTools
{
    ...
    [Description("Get current and available balances for a customer's accounts, optionally filtered to one account.")]
    public static string GetAccountBalance(
        [Description("The customer ID, e.g. CUST-1001.")] string customerId,
        [Description("Optional account ID to filter to a single account.")] string? accountId = null)
    {
    ...
    [Description("List a customer's most recent transactions, newest first, optionally filtered to one account.")]
    public static string GetRecentTransactions(
        [Description("The customer ID, e.g. CUST-1001.")] string customerId,
        [Description("Optional account ID to filter to a single account.")] string? accountId = null,
        [Description("Maximum number of transactions to return.")] int limit = 5)
    {
    ...
    [Description("List all of a customer's accounts with a combined total balance.")]
    ...
    [Description("Look up a customer's profile: name, contact details, and member-since date.")]
    ...
    [Description("Get current loan APRs, optionally filtered by product type: auto, home, or personal.")]
    public static string GetLoanRates(
        [Description("Optional product type: auto, home, or personal.")] string? productType = null)
    {
        var rates = new[]
        {
    ...
    [Description("Calculate the monthly payment, total interest, and total cost for a loan.")]
    public static string CalculateLoanPayment(
        [Description("Loan principal in dollars.")] double principal,
        [Description("Annual interest rate as a percentage, e.g. 4.99.")] double annualRate,
    ...
    [Description("Search the banking FAQ knowledge base for answers to general questions.")]
    public static string SearchFaq(
        [Description("The customer's question or keywords to search for.")] string query)
    {
